/**
 * SwipePager - Touch gesture handler for mobile navigation
 *
 ...
 *
 * Features:
 * - Pointer Events with touch detection
...
const swipeLogger = logger.createChild('SwipePager');
...
export class SwipePager {
  constructor({ onPrev, onNext, logger } = {}) {
    ...
    this.logger.info('SwipePager initialized', this.config);
  }
  ...
  attach(el) {
    ...
    this.logger.info('SwipePager attached to element');
  }
  ...
  _onPointerUp = (e) => {
    ...
    const validTime = dt <= this.config.maxTime;
    ...
    if (validDistance && validTime && validVelocity) {
      ...
      if (dx < 0) {
        this.logger.info('Swipe left detected (dx=' + dx + ') - triggering next');
        this.onNext?.();
      } else {
        this.logger.info('Swipe right detected (dx=' + dx + ') - triggering prev');
        this.onPrev?.();
      }
    } else {
  ...
  updateConfig(newConfig) {
    ...
    this.logger.debug('SwipePager config updated', this.config);
  }
  ...
  destroy() {
    ...
    this._reset();
    this.el = null;
    this.logger.info('SwipePager destroyed');
  }
}
